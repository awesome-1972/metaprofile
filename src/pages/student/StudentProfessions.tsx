...
import { 
  ...
  BookOpen, Briefcase, ChevronRight
} from "lucide-react";
        ...
        {!selectedArea ? (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className="lg:col-span-2 space-y-6">
              ...
              {/* Career Path */}
              <div className="rounded-lg border border-border bg-card p-6">
                <h3 className="font-medium text-foreground mb-4">Кар'єрний шлях</h3>
                <div className="flex items-center gap-4">
                  {["Junior", "Middle", "Senior", "Lead"].map((level, index) => (
                    <div key={level} className="flex-1 text-center">
                      <div className={`h-2 rounded-full mb-2 ${index === 0 ? "bg-primary" : "bg-border"}`} />
                      <p className={`text-sm font-medium ${index === 0 ? "text-primary" : "text-muted-foreground"}`}>{level}</p>
                      <p className="text-xs text-muted-foreground">{index === 0 ? "Старт" : `${index * 2}-${index * 2 + 2} роки`}</p>
                    </div>
                  ))}
                </div>
              </div>
