...
import { useVacancyCompetencies, groupCompetencies } from "@/hooks/ats/use-competencies";
import {
  useCompetencyScores,
                      ...
                      {group.competencies.map((c) => {
                        ...
                        return (
                          <div key={c.id} className="border rounded-md p-3 space-y-2">
                            ...
                            <RadioGroup
                              value={scores[c.id] !== undefined ? String(scores[c.id]) : undefined}
                            ...
                            <Textarea
                              placeholder="Нотатка інтерв'юера (необов'язково)"
                              value={notes[c.id] ?? ""}
