# Хендоф — ATS Metaprofile (стан на 2026-07-13)

## Сесія 13.07 — Перебудова процесу: 6 етапів пошуку, кожен зі своєю воронкою

Найбільша зміна процесу з моменту запуску ATS. Замість плаского kanban на вакансію —
**двоповерхова модель**: етап пошуку → його власна воронка стадій. Зроблено 4 спринти,
усе в `main` (останній коміт `dc858b5`), БД синхронна, edge-функції задеплоєні.

Деталі моделі й рішень — `docs/process-search-phases.md` (єдине джерело правди по процесу).

### Стан: усе застосовано, нічого не висить
- Коміти: `b0fee41` (етапи) → `8258528` (шаблони листів) → `07da2e0` (стратегія + бріф) →
  `f056f84` (імпорт Excel) → `a02eb49`, `dc858b5` (фікси + lint).
- Міграції в проді (усі 4): `20260713090000` search_phases, `20260713100000` message_templates,
  `20260713110000` preparation_artifacts, `20260713120000` longlist_import.
- Edge задеплоєні: **draft-communication** (AI-чернетка листа), **generate-public-brief**
  (AI-бріф для кандидатів). Секрети ті самі: `ANTHROPIC_API_KEY`, `ANTHROPIC_MODEL`.
- Нова npm-залежність: **xlsx** (SheetJS) — якщо клонувати на іншу машину, `npm install`.
- Lint: **0 errors** (було 23). Лишилось 6 warnings — усі `react-hooks/exhaustive-deps`
  у V1-демо (BriefTab ×2, ColleagueChat, useVirtualInterview, CaseWorkPage, CompanyDashboard).
  Це не косметика: додавання залежностей може дати нескінченний re-render — чіпати окремо,
  з перевіркою поведінки.

### Що тестувати завтра (наскрізь на CFO-проєкті)
1. **Етап 1 «Підготовка»** (вкладка «Етапи пошуку» → етап 1):
   - чеклист 5 пунктів (requisition / бріф / матриця / стратегія / бріф кандидатам);
   - план проекту = планові дати всіх 6 етапів;
   - **Стратегія пошуку**: фокус, галузі з частками (лічильник до 100%), цільові компанії
     й посади → «PDF» (друк брендованого HTML, лого з `/logo.png`);
   - **Бріф для кандидатів**: «AI-чернетка з внутрішнього бріфу» → редактор секцій →
     «Затвердити» → PDF. Клієнт за замовчуванням НЕ називається (чекбокс розкриття).
     Якщо внутрішнього бріфу й опису вакансії нема — функція чесно віддає `brief_empty`.
2. **Етап 2 «Лонг-лист»**: кнопка «Імпорт лонг-листа з Excel» → прев'ю плану.
   Дивитись: чи розпізнались колонки (Компанія/ПІБ/Посада/Досвід/Соцмережі/Status/Коментарі),
   чи правильно класифіковано «відмова кандидата» vs «відмова агенції», чи не наплодило дублів
   (дедуп: email, якщо порожній — точний ПІБ). Наявні поля кандидата НЕ затираються.
   Поряд — перемикач **Kanban / Таблиця** (таблиця = редагування рядками, як в Excel).
3. **Дії на картці**: «Далі» (лист-запрошення + переведення на стадію) і «Відмова»
   (причина обовʼязкова, чекбокс «без листа»). У обох — «AI-чернетка» персоналізує текст.
   Перевірити, що лист реально дійшов (Resend) і що відмовлений зник із колонки (лічильник −N).

### Ймовірні місця, де тре
- Парсер Excel: якщо шапка нестандартна — колонка не розпізнається (мапимо за синонімами назв,
  не за позицією). Додати синонім у `COLUMN_SYNONYMS` (`src/lib/ats/longlist-parser.ts`).
- Класифікація Status: регулярка в `classifyStatus` (там само) — при нових формулюваннях
  доповнити.
- Стадії за замовчуванням (21 шт.) — у шаблоні «MetaVision — 6 етапів пошуку»
  (`pipeline_stage_templates`). Правити стадії конкретної вакансії можна прямо в БД;
  UI-редактора стадій поки немає — це кандидат на наступний спринт.

### Пастки середовища (не змінилось, але коштувало часу)
- **`supabase db push` падає** на `permission denied to alter role cli_login_postgres` —
  це автентифікація CLI, не SQL. Обхід: `$env:SUPABASE_DB_PASSWORD="<пароль БД>"` перед push,
  або SQL руками в Dashboard + рядок у `supabase_migrations.schema_migrations`.
  **Перед SQL звіряти проєкт**: ATS = `mnpcevhzqgcrllymdmil`, хаб = `vpgdjffmcnkqgwqdrsyd`.
- Пісочниця Claude віддає **обрізані копії** файлів із українськими коментарями — фантомні
  syntax errors. Реальні файли на хості коректні; комітити з пісочниці не можна.
