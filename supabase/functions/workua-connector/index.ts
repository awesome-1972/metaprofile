...
// Нормалізація значень довідників work.ua (category/jobtype/experience/education),
// які API віддає російською (легасі), → українська. Невідомі лишаються як є.
const RU_UA_TERM: Record<string, string> = {
  // experience
  "готовы взять без опыта": "готові взяти без досвіду",
  "без опыта": "без досвіду",
  "от 1 года": "від 1 року",
  "от 2 лет": "від 2 років",
  "от 5 лет": "від 5 років",
  // education
  "высшее": "вища",
  "неоконченное высшее": "незакінчена вища",
  "среднее специальное": "середня спеціальна",
  "среднее": "середня",
  "не важно": "не важливо",
  // jobtype
  "полная занятость": "повна зайнятість",
  "неполная занятость": "неповна зайнятість",
  "удаленная работа": "віддалена робота",
  "дистанционная работа": "дистанційна робота",
  "сменный график": "змінний графік",
  "стажировка": "стажування",
  "сезонная работа": "сезонна робота",
  "работа за рубежом": "робота за кордоном",
  // category (стандартний перелік work.ua)
  "IT, компьютеры, интернет": "IT, комп'ютери, інтернет",
  "Администрация, руководство среднего звена": "Адміністрація, керівництво середньої ланки",
  "Банки, инвестиции, лизинг": "Банки, інвестиції, лізинг",
  "Без опыта работы, студенты": "Без досвіду роботи, студенти",
  "Начало карьеры, студенты": "Початок кар'єри, студенти",
  "Бухгалтерия, аудит": "Бухгалтерія, аудит",
  "Гостинично-ресторанный бизнес, туризм": "Готельно-ресторанний бізнес, туризм",
  "Дизайн, творчество": "Дизайн, творчість",
  "Здравоохранение, медицина, фармацевтика": "Охорона здоров'я, медицина, фармацевтика",
  ...
  "Красота, фитнес, спорт": "Краса, фітнес, спорт",
  "Культура, музыка, шоу-бизнес": "Культура, музика, шоу-бізнес",
  "Логистика, склад, ВЭД": "Логістика, склад, ЗЕД",
  "Маркетинг, реклама, PR": "Маркетинг, реклама, PR",
  "Недвижимость": "Нерухомість",
  "Образование, наука": "Освіта, наука",
  "Охрана, безопасность": "Охорона, безпека",
  "Продажи, закупки": "Продаж, закупівлі",
  "Производство, инженерия": "Виробництво, інженерія",
  "Рабочие специальности, производство": "Робочі спеціальності, виробництво",
  "Розничная торговля": "Роздрібна торгівля",
  "Секретариат, делопроизводство, АХО": "Секретаріат, діловодство, АГВ",
  "Сельское хозяйство, агробизнес": "Сільське господарство, агробізнес",
  "СМИ, издательство, полиграфия": "ЗМІ, видавництво, поліграфія",
  "Страхование": "Страхування",
  "Строительство, архитектура": "Будівництво, архітектура",
  "Телекоммуникации, связь": "Телекомунікації, зв'язок",
  "Транспорт, автобизнес": "Транспорт, автобізнес",
  "Управление персоналом, HR": "Управління персоналом, HR",
  "Финансы, экономика": "Фінанси, економіка",
  "Химическая промышленность": "Хімічна промисловість",
  "Юристы": "Юристи",
  "Юриспруденция": "Юриспруденція",
  "Другие сферы деятельности": "Інші сфери діяльності",
};
const uaTerm = (name: string) => RU_UA_TERM[name] ?? RU_UA_TERM[name.toLowerCase()] ?? name;
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === "dictionaries") {
      ...
      const ua = (arr: { id: string; name: string }[]) => arr.map((o) => ({ id: o.id, name: uaTerm(o.name) }));
      const town = pick("town").map((t: { id: string; name: string }) => ({ id: t.id, name: RU_UA_CITY[t.name] ?? t.name }));
      return json({
        town,
        category: ua(pick("category")),
        jobtype: ua(pick("jobtype")),
        experience: ua(pick("experience")),
        education: ua(pick("education")),
        publication_type: ua(pick("publication_type")),
      });
    }
